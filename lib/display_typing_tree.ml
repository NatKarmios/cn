...
module Core_level = struct
  let rec display ~backup_loc : Context.t Or_TypeError.t t -> DT.t = function
    | End (Ok ctx) ->
      let state = lazy (display_context ~backup_loc ctx) in
    ...
    | End (Error e) ->
      let state =
        ...
        | Some (ctx, _) -> lazy (display_context ~backup_loc ctx)
        | None -> lazy empty_state
      in
      let get_state () = Lazy.force state in
...
module Lifted = struct
  ...
  and display_breakpoint dctx = function
    | Bp (Core_step (expr, ctx)) ->
      let* stmt =
        let cabs_index, _ = dctx in
        match get_origin cabs_index expr with
        | None | Some (CI.OExpr _) -> None
        | Some (CI.OStmt cabs_stmt) -> Some cabs_stmt
      in
      let msg = Fmt.str "%a" pp_stmt stmt in
      let (Cabs.CabsStatement (loc, _, _)) = stmt in
      let state = lazy (display_context ~loc ctx) in
      let get_state () = Lazy.force state in
      Some (DT.step get_state msg)
    | Bp (Nest n) -> Some (DT.nest [ display_next dctx n ])
    | Bp Step_in -> None
    | Bp Step_out -> None
